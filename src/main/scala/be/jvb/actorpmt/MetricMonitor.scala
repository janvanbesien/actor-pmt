...
class MetricMonitor(val sourceMetricDefinition: MetricDefinition, /* TODO: do this better... repo... */ val monitorsByDependencies: mutable.MultiMap[MetricDefinition, MetricMonitor]) extends Actor with MetricProvider
{
...

  {
    ...
    for (dependency <- sourceMetricDefinition.dependencies) {
      received.put(dependency, new mutable.HashMap[Interval, mutable.Set[Metrics]] with mutable.MultiMap[Interval, Metrics])
    }
  }

  def dependants = {
    // go find all monitors that depend on us
    monitorsByDependencies.get(sourceMetricDefinition) match {
  ...
  def processMetricAvailableMessage(receivedMessage: MetricAvailableMessage) = {
    ...
    println("monitor [" + sourceMetricDefinition + "] received something which it accounts in monitor interval [" + accountingInterval + "]")
    ...
    if (allDependenciesReceived(accountingInterval)) {
      println("monitor [" + sourceMetricDefinition + "] received all its dependencies at [" + new DateTime + "], calculating and providing metrics")
      ...
      println("monitor [" + sourceMetricDefinition + "] didn't receive all its dependencies at [" + new DateTime + "]")
    }
  }
  ...
  def calculateIntervalInWhichToAccount(receivedMessage: MetricAvailableMessage): Interval = {
    ...
    val accountingIntervalStart = DateTimeUtilities.alignOnPreviousOrEqual(sourceMetricDefinition.granularity, receivedMessage.metrics.interval.start)
    return new Interval(accountingIntervalStart, accountingIntervalStart + sourceMetricDefinition.granularity)
  }
    ...
    for ((dependency, metricsPerMonitorInterval) <- received.elements) {
      ...
      alreadyReceivedMetricsInMonitorInterval match {
        ...
        case Some(receivedMetrics) => { // something received, check if everything is received
          ...
          println("monitor [" + sourceMetricDefinition + "] expects [" + dependency + "] in intervals [" + expectedProviderIntervals + "] and has [" + receivedMetrics + "]")
  ...
  def calculateMetrics(sourceMetrics:Metrics, accountingInterval:Interval) : Metrics = {
    ...
    return new Metrics(sourceMetricDefinition, immutable.Map() ++ sourceMetrics.valuesPerManagedObject.map{case (key,value) => (key,value*2)}, accountingInterval)
  }
}
